...
engine = create_engine('mysql+mysqlconnector://', executor=dump, strategy="mock")
...
class FactoryCar(factory.alchemy.SQLAlchemyModelFactory):
    ...
    name = "Beetle"
    drivetrain = "FWD"
    maker_id = factory.RelatedFactory(FactoryCompany, factory_related_name="company_id")
    price=250000
...
class FactoryHuman(factory.alchemy.SQLAlchemyModelFactory):
    class Meta:
        ...
        sqlalchemy_session = session
    name = "Dick"
    credit_card_id = factory.RelatedFactory(FactoryCreditCard, factory_related_name="card_id")
    
class FactoryOwnership(factory.alchemy.SQLAlchemyModelFactory):
    class Meta:
        model = Ownership
        sqlalchemy_session = session
    human_id = factory.RelatedFactory(FactoryHuman, factory_related_name="id_number")
    car_id = factory.RelatedFactory(FactoryCar, factory_related_name="car_id")


def query_example_model(engine):
    pass
