...
class LogGroup:
    ...
    creationTime: int = field(default=None)
    kmsKeyId: str = field(default=None)
    logGroupName: str = field(default=None)
    metricFilterCount: int = field(default=None)
    retentionInDays: int = field(default=None)
    storedBytes: int = field(default=None)
...
class LogStream:
    arn: str = field(default=None)
    creationTime: int = field(default=None)
    firstEventTimestamp: int = field(default=None)
    lastEventTimestamp: int = field(default=None)
    lastIngestionTime: int = field(default=None)
    logStreamName: str = field(default=None)
    storedBytes: int = field(default=None)
    uploadSequenceToken: str = field(default=None)
...
def create_log_group(name, tags: dict = None) -> str:
...
def put_retention_policy(
    log_group: str, retention_option: int = -1
) -> Tuple[str, int]:
    action = "PutRetentionPolicy"
    params = dict(
        logGroupName=log_group,
    )
    if retention_option in [
        1,
        3,
        5,
        7,
...
def get_log_events(log_group, log_stream, start_time=0, end_time=0):
    action = "GetLogEvents"
    ...
    if 200 <= res.status_code < 300:
        logging.debug(res.text)
        resp_body = res.json()
        log_events = resp_body.get("events")
