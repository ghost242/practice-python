...
def build_agents(
    ...
    *,
    ...
    """
    Build agents for a browser-only DRM-protected EPUB rental service project.

    model_names:
    - model_names[0]: reply model
    - model_names[1]: summary/evaluation/subtopic model
    """
...
def main() -> None:
    ...
    model_names = [
        "phi4:latest",  # reply model
        "ministral-3:8b",  # summary/evaluation/subtopic model
    ]
    agent_max_turns = 20
    ...
    state = run_multi_agent_chat_sync(
        ...
        max_turns_per_subtopic=20,
        host_llm=decider_llm,
        export_path="discussion.json",
    )

    print("\n=== FINAL TURNS ===")
    for turn in state.turns:
        print(
            f"[{turn.turn_index}] {turn.participant_id}/{turn.participant_type}: {turn.content}"
        )
    ...
    for participant_id, runtime in state.participant_states.items():
        print(
            ...
            f"summarie={runtime.summary_text}"
        )
